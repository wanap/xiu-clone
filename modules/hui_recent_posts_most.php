    ...
    if( !_hui('most_list_style') || _hui('most_list_style')=='comment' ){
        ...
        if($most_viewed) {
            ...
            foreach ($most_viewed as $post) {
                ...
                // $output .= '<li class="item-'.$i.'"><a target="_blank" href="'.get_permalink($postid).'">'._get_post_thumbnail(array()).'<h2>'.$post_title.'</h2><p>'.timeago( get_the_time('Y-m-d H:i:s') ).'<span class="post-views">阅读('.$post_views.')</span></p></a></li>';
