...
function hui_breadcrumbs(){
    ...
    return __('<span class="text-muted">当前位置：</span>', 'haoui').'<a href="'.get_bloginfo('url').'">'.get_bloginfo('name').'</a> <small>></small> '.get_category_parents($category->term_id, true, ' <small>></small> ').'<span class="text-muted">文章详情</span>';
}
...
function hui_posts_related($title='', $limit=8){
    ...
    if ( $i < $limit ) { 
        ...
        query_posts($args);
        while( have_posts() ) { the_post();
            echo '<li><a'.hui_target_blank().' href="'.get_permalink().'">'.hui_get_thumbnail().'<p>'.get_the_title().'</p></a></li>';
            $i ++;
        };
        wp_reset_query();
    }
...
function hui_the_content(){
    global $post;
    $content = $post->post_content;
    $tmp = str_replace('src', 'data-original', $content);
    $html = str_replace('<img ', '<img class="thumb" ', $tmp);
    return $html;
}
...
function timeago( $ptime ) {
    ...
    $interval = array (
        ...
        30 * 24 * 60 * 60       =>  __('个月前', 'haoui').' ('.date('Y-m-d', $ptime).')',
        7 * 24 * 60 * 60        =>  __('周前', 'haoui').' ('.date('Y-m-d', $ptime).')',
